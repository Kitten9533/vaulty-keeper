...
// "insight", "rediscli", "mongosh") that callers map to a localized label; Value is the
...
// authenticates against (PG/MySQL username, Redis/MongoDB password); the other
// field is a fixed placeholder ("x", or "vaulty" for MongoDB).
func RawTunnelURL(typ, token, host string, port int, db string) string {
	...
	case "mongodb":
		...
		u := url.URL{Scheme: "mongodb", User: url.UserPassword("vaulty", token),
			Host: net.JoinHostPort(host, strconv.Itoa(port)), Path: "/" + db, RawPath: "/" + url.PathEscape(db),
			RawQuery: "authSource=admin&authMechanism=SCRAM-SHA-256&directConnection=true&retryWrites=false"}
		return u.String()
	}
	return ""
}
...
// a connection. typ is "postgres", "mysql", "redis" or "mongodb"; host is 127.0.0.1 or
...
func TunnelLinks(typ, token, host string, port int, db string, tr func(key string, args ...any) string) (raw string, links []ClientLink, err error) {
	...
	case "mongodb":
		links = []ClientLink{{Kind: "mongosh", Value: "mongosh '" + strings.ReplaceAll(raw, "'", "'\"'\"'") + "'"}}
	default:
