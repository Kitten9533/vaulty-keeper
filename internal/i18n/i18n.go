...
var dict = map[string][2]string{
	...
	"cli.too-many-args":           {"unexpected extra argument %q", "多余参数 %q"},
	"cli.appid-conflict":          {"positional appid %q conflicts with --appid %q", "位置参数 appid %q 与 --appid %q 冲突"},
	"cli.names-reveal-conflict":   {"--names cannot be combined with --reveal", "--names 不能与 --reveal 同时使用"},

	// ---- usage lines (fail "用法：") ----
	"usage.apollo.list":    {"usage: vaulty-keeper apollo list [<env> [<appid>]] [--appid <id>] [--names] [--json]", "用法：vaulty-keeper apollo list [<env> [<appid>]] [--appid <id>] [--names] [--json]"},
	...
	"usage.apollo.export":  {"usage: vaulty-keeper apollo export <name> [<appid>]", "用法：vaulty-keeper apollo export <name> [<appid>]"},
	"usage.apollo.rm":      {"usage: vaulty-keeper apollo rm <name> [<appid>] [--appid <id>]", "用法：vaulty-keeper apollo rm <name> [<appid>] [--appid <id>]"},
	"usage.apollo.reveal":  {"usage: vaulty-keeper apollo reveal <name> <key...>", "用法：vaulty-keeper apollo reveal <name> <key...>"},
	"usage.apollo.edit":    {"usage: vaulty-keeper apollo edit <name> [<appid>]", "用法：vaulty-keeper apollo edit <name> [<appid>]"},
	...
	"help.d.apollo.list":    {"without <env> list all snapshots (--json prints the catalog); with <env> [<appid>] list that snapshot; --names prints key names without decrypting", "不带 <env> 列出所有快照（--json 输出 catalog JSON）；带 <env> [<appid>] 列出该快照；--names 只打 key 名、不解密"},
	...
	"help.d.db.add":         {"register a connection (URL read from stdin, encrypted at rest; tunnel stays off until db on; --test verifies first)", "注册连接（URL 从 stdin 读，加密落盘；隧道默认关闭，需 db on 才开启；--test 先验证）"},
	...
	"help.d.db.on":          {"turn a connection's tunnel on (off after db add; serve picks it up within ~2s)", "开启连接的隧道（db add 后默认关闭；serve 约 2 秒内生效）"},
	...
	"help.usage.db":        {"Database URLs are encrypted at rest (dedicated DB key) and never shown to\nscripts/AI. 'vaulty-keeper serve' opens one TCP tunnel per connection that is\nexplicitly turned on; inside an isolated container use native clients\n(psql/mysql/redis-cli) against the tunnel port, with the token as the\nusername/AUTH password — the token is not the real database password. Tunnels\nstay off after 'db add' until 'db on' (or the UI Turn on button); 'db off'\ncloses them (serve picks the change up within ~2s). 'db test' verifies a\nregistered URL authenticates (without printing it); 'db show' prints only on\nyour own terminal.\n", "数据库 URL 加密落盘（独立 DB 密钥），永不向脚本/AI 显示。'vaulty-keeper serve'\n只为已显式开启的连接起 TCP 隧道；隔离容器内用原生客户端（psql/mysql/redis-cli）\n连隧道端口，token 作用户名/AUTH 密码——token 不是真实数据库密码。\n'db add' 后隧道保持关闭，需 'db on'（或 UI「开启隧道」）才监听；'db off' 关闭\n（serve 约 2 秒内生效）。'db test' 验证注册的 URL 可认证（不打印它）；\n'db show' 只在你自己的终端打印。\n"},
	...
	"db.added":                  {"connection %q (%s) saved encrypted; tunnel is off until 'vaulty-keeper db on %s'", "连接 %q（%s）已加密保存；隧道默认关闭，执行 'vaulty-keeper db on %s' 后才开启"},
