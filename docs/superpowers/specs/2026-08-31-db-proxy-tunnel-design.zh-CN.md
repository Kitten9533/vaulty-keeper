...
[English](2026-08-31-db-proxy-tunnel-design.md) | 中文

> **历史三协议设计来源，2026-08-31；生命周期标注于 2026-09-07。非可执行说明。** 保留当时确认的取舍、拟议协议/依赖和测试要求，不追认全部已实现。现行用法和边界由[数据库架构](../../db-proxy-architecture.zh-CN.md)与[示例](../../db-proxy-examples.zh-CN.md)维护；[历史实施计划](../plans/2026-08-31-db-proxy-tunnel.zh-CN.md)保留演进线索。
>
> **已被替代的假设：** 当前 PostgreSQL 前端为 trust 风格，MySQL 使用手写握手而非 go-mysql 依赖；端口在注册时分配，不探测 OS 占用。容器使用 `remote dblist`，连接命令由有本地密钥的宿主生成。stdin 不消除上游 shell history，token 不抵消明文网络风险，裸转发也不保证业务结果/后端错误无秘密；MySQL TLS 仍需按现行指南限定。全局/专属 token 与既有会话语义见现行架构，不沿用此处单 token 提案。Mongo 排除条款仅属于 2026-08-31 范围，已由 [Mongo 设计](2026-09-07-mongodb-tunnel-design.zh-CN.md)接续，当前验证见 [Mongo 指南](../../mongodb-tunnel-guide.zh-CN.md)。不得执行下文注册、取钥匙或客户端示例。

日期：2026-08-31
当时状态：已确认（用户逐项拍板；仅记录当时决策，不是后续会话授权）
