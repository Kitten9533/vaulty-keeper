...
[English](2026-08-25-full-ui-migration-design.md) | 中文

> **历史设计来源，2026-08-25；生命周期标注于 2026-09-07。不可作为执行计划。** 保留原始取舍、拟议 API 和测试要求，不追认全部已实现。现行操作及安全边界见 [UI 指南](../../ui-guide.zh-CN.md)和 [Apollo 指南](../../apollo-snapshot-guide.zh-CN.md)；相关[历史计划](../plans/2026-08-25-full-ui-migration.zh-CN.md)保留代码来源。
>
> **已被替代的假设：** 下文暂缓 AI 明文边界、保留 TTY 菜单、可选 AppID、`/api/aes/config` 路由及所有明文操作均二次确认的描述不是现行约定。TTY 菜单已移除，快照身份后继为 [Env+AppID 设计](2026-08-25-env-appid-and-snapshot-delete.zh-CN.md)。旧 `confirm:true` 字段不证明人工确认；旧密钥初始化/明文示例不得重放。保留共享 `internal/app` 层的设计动机，不据此恢复已移除路由或放宽权限。
