...
[English](2026-08-31-db-proxy-tunnel.md) | 中文

> **历史计划来源，2026-08-31；生命周期标注于 2026-09-07。不可执行或重放。** 保留完整任务清单、拟议实现和测试要求；未勾选项不是当前待办，也不补勾为通过。现行[数据库架构](../../db-proxy-architecture.zh-CN.md)与[用法示例](../../db-proxy-examples.zh-CN.md)接管操作说明。[设计](../specs/2026-08-31-db-proxy-tunnel-design.zh-CN.md)顶部列出历史与现行差异，包含 PG 前端认证、go-mysql 依赖、token 与端口语义。
>
> 原计划要求使用 `subagent-driven-development` 或 `executing-plans` 逐项执行；该工作流要求仅属于当时会话，现已失效。Mongo 排除条款仅是当时范围；后继见 [Mongo 设计](../specs/2026-09-07-mongodb-tunnel-design.zh-CN.md)与[当前指南及验证](../../mongodb-tunnel-guide.zh-CN.md)。不得执行这里的真实连接、密钥、构建或测试示例来“完成”旧清单。
...
参考历史 [spec](../specs/2026-08-31-db-proxy-tunnel-design.zh-CN.md)。
