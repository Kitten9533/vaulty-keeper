...
Recorded, not repeated here: the dated MongoDB 8.0.13 standalone/fixed-replica-set matrix (unit/race/vet/build, native Go driver, container `mongosh`) lives in [mongodb-tunnel-guide.md](tunnel/mongodb-tunnel-guide.md#verification-status) and is historical — not a new run during documentation work.
