...
**MySQL `?tls=true`：** 设置 `tls=true` 时代理会声明 `CLIENT_SSL` 能力位，将后端连接升级为 TLS，并用升级后的连接完成认证和原始转发。如需信任私有/自签 CA，加 `tlsCAFile=<路径>`（PEM CA 文件，≤1 MiB）；不指定时按系统根证书验证后端证书。证据与缺口见[安全模型验证状态](security-model.zh-CN.md#8--验证状态)。明文模式不变。PostgreSQL/Redis TLS：[PostgreSQL 指南](tunnel/postgres-tunnel-guide.zh-CN.md) / [Redis 指南](tunnel/redis-tunnel-guide.zh-CN.md)。

## Token 与监听

| 控制 | 当前效果 |
|---|---|
| `db add` | 生成加密存储的 128 位随机专属 token，`enabled` 为 false，需 `db on` / UI「开启隧道」后才监听 |
...
| 同名 `db add` | 替换 URL、生成新 token、把 `enabled` 重置为 false；未指定 `--port` 时保留已存端口。需重新分发 token，若要监听须再次显式开启 |
...
**`scripts/dbtest.sh` 已隔离，可以安全运行。** 隔离细节与历史对比见[安全模型验证状态](security-model.zh-CN.md#8--验证状态)和 [CONTRIBUTING](../CONTRIBUTING.zh-CN.md)。镜像与未执行的合成步骤见[示例](db-proxy-examples.zh-CN.md)。
...
本指南描述工作区实现。最新发布 **v0.9.0** 已包含当前 DB 隧道行为（新注册默认关闭，需 `db on`）并打包 `docs/` 指南；预编译 0.6.0 归档先于两者，且缺少所链接的 `docs/`。本次文档更正未做真实 TLS 测试或人工 TTY 检查。带日期的历史 Mongo 验证矩阵仅由 [Mongo 指南](tunnel/mongodb-tunnel-guide.zh-CN.md#验证状态) 维护。
