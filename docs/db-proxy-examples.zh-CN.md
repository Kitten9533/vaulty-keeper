...
适用于当前工作区的 PG/MySQL/Redis。下方完整合成步骤已按源码核对，2026-09-07 文档更正期间**没有执行**；预期结果不是测试输出。配套见[架构](db-proxy-architecture.zh-CN.md)及统一[安全模型](security-model.zh-CN.md)。MongoDB 8 有独立[指南和历史验证矩阵](mongodb-tunnel-guide.zh-CN.md)：固定单端点、`vaulty` 加专属密码 token、无全局兜底、持续命令/元数据检查，不承诺完整 GUI 支持或即时撤销会话。
