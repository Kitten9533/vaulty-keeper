...
This guide describes the working-tree implementation. The latest release **v0.8.0** includes the current DB tunnel and MongoDB behavior and bundles the `docs/` guides; prebuilt 0.6.0 archives predate both and lack the linked `docs/` files. No release/package generation, runtime test, real TLS test or manual TTY check was performed for these documentation corrections. The dated historical Mongo validation matrix is maintained only in the [Mongo guide](tunnel/mongodb-tunnel-guide.md#verification-status).
