...
Every user-facing doc is English-default with a `.zh-CN.md` sibling linked at its top. The database tunnel is the flagship feature: architecture and fixtures live in the `db-proxy-*` guides, and per-protocol setup lives under `docs/tunnel/`.
...
This README describes the current source workspace. The latest release is **v0.9.0** (2026-09-08): new `db add` registrations stay off until `db on` / UI Turn on; legacy `db.json` entries with neither `enabled` nor `disabled` stay on. Archives bundle both READMEs, LICENSE, AGENTS.md, CONTRIBUTING.md, SECURITY.md and the `docs/` guides (index, security model, cli-reference, container-isolation, ui-guide, and per-protocol tunnel guides, English + Chinese). Historical implementation records are archived under git tag `docs-superpowers-archive`, not in this tree or the archives. Older archives such as **0.6.0** do not include `docs/`; for those, browse the matching tag's `docs/` in the [source repository](https://github.com/Kitten9533/vaulty-keeper). Guides in this workspace are not evidence for binaries older than v0.9.0.
...
- **Tunnels stay off after `db add`** (`enabled` defaults to false in `db.json`); `db on <name>` (or the UI Turn on button) opens a listener on the next watcher sync. `db off <name>` / `db off --all` close them. List output shows off state.
...
- Same-name `db add` retains the port if omitted, but creates a fresh token and resets `enabled` to false. Redistribute client links and turn the tunnel on again if it should listen. UI on/off is saved configuration, not listener/backend health; `Broken` means registered URL decryption failed, while token decryption can fail separately during resolution.
...
- **Backend TLS**: PG `sslmode` is handled by the proxy itself, not libpq (three force-TLS names share one hostname-verifying TLS dial; the frontend refuses SSLRequest — use `sslmode=disable` on the tunnel URI). See the [PostgreSQL guide](docs/tunnel/postgres-tunnel-guide.md). Redis uses `rediss://`. **MySQL `?tls=true` negotiates TLS with the backend**; add `tlsCAFile=<path>` to trust a private/self-signed CA. MongoDB implements certificate/hostname verification for `tls=true`/`ssl=true` with optional `tlsCAFile`; actual MongoDB TLS remains unverified. Evidence and remaining gaps: [security model](docs/security-model.md#8--verification-status). Client-to-proxy transport is plaintext; use localhost or an isolated trusted network.
...
- `db add` — writes an encrypted URL and fresh token with `enabled=false`; stdin does not erase upstream history or terminal echo. Same-name registration resets the token and turns the tunnel off. Real URLs must be supplied by a trusted human, not retrieved by an agent.
